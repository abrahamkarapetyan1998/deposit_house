...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
        ...
        if (!empty($slug1)) {
            $subcat = Subcategory::where('slug', $slug1)->where('category_id',$cat->id)->firstOrFail();
            $data['subcat'] = $subcat;
        }
        if (!empty($slug2)) {
            $childcat = Childcategory::where('slug', $slug2)->where('subcategory_id',$subcat->id)->firstOrFail();
...
->when($cat, function ($query, $cat) {
    ...
    ->when($childcat, function ($query, $childcat) {
        return $query->where('childcategory_id', $childcat->id);
            ...
            ->when($sort, function ($query, $sort) {
                if(in_array($sort, ['date_asc', 'date_desc'])) {
                    return $query->orderBy('id', $sort == 'date_asc' ? 'ASC': 'DESC');
                }

                if(in_array($sort, ['price_asc', 'price_desc'])) {
                    return $query->orderBy('price', $sort == 'price_asc' ? 'ASC': 'DESC');
                }
            })
    ...
    public function product($slug)
    {
        ...
        ProductClick::Click_Sale_Count($productt->id,'click');
