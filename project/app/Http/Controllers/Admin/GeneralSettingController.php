...
class GeneralSettingController extends Controller
{
    ...
    public function generalupdate(Request $request)
    {
        ...
        $input = $request->all();
        if($request->language_popup){
            $input['popup_langauge'] = json_encode($request->language_popup);
        }
        
        if($request->language){
            $input['langauge'] = json_encode($request->language);
        }
        
        if($request->footer_language){
            $input['footer_language'] = json_encode($request->footer_language);
        }

        if($request->contact_langauge){
            $input['contact_langauge'] = json_encode($request->contact_langauge);
        }
    ...
    public function istrmas($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_trams = $status;
        $data->update();
        cache()->forget('generalsettings');
    }

    public function sms_verification($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_sms_verification = $status;
        $data->update();
        cache()->forget('generalsettings');
    }
    public function is_sms($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_sms = $status;
        $data->update();
    ...
    public function cpay($status)
    {

        $data = Generalsetting::findOrFail(1);
        $data->cpay_check = $status;
        $data->update();
        cache()->forget('generalsettings');
    }
