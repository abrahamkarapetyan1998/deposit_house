...
class ProductController extends Controller
{
    ...
    {
    	 $user = Auth::user();
         $datas = $user->products()->where('product_type','normal')->orderBy('id','desc');
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Product $data) {
                                ...
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.$this->languages->lang713.'</option><<option data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.$this->languages->lang714.'</option>/select></div>';
                            })
                            ->addColumn('action', function(Product $data) {
                                ...
                                return '<div class="godropdown"><button class="go-dropdown-toggle">'.$this->languages->lang256.'<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('vendor-prod-edit',$data->id) . '"> <i class="fas fa-edit"></i>'.$this->languages->lang111.'</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i>'.$this->languages->lang716.'</a><a href="javascript:;" data-href="' . route('vendor-prod-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i>'.$this->languages->lang261.'</a></div></div>';})
    ...
    public function catalogdatatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function(Product $data) {
                                ...
                                $catalog = $ck ? '<a href="javascript:;"> Added To Catalog</a>' : '<a href="' . route('vendor-prod-catalog-edit',$data->id) . '"><i class="fas fa-plus"></i>'.$this->languages->{'add to catalog'}.'</a>';
    ...
    public function importSubmit(Request $request)
    {
        ...
        $customs = [
            'csvfile.required' => __('The csvfile field is required.'),
            'csvfile.mimes' => __('The csvfile must be a csv, text.')
        ];
...
//                $input['childcategory_id'] = $request->childcategory_id;
                            ...
                            $saveto = public_path() . 'assets/images/products/' . $fphoto;
                            ...
                            if (strpos($contentType, 'image/') !== false) {
                                ...
                                $fimg = Image::make($saveto)->resize(800, 800);
                            ...
                            $timg = Image::make($saveto)->resize(285, 285);
                            $thumbnail = Str::random(12) . '.jpg';
        ...
        //--- Redirect Section
        $msg = ''.__('Bulk Product File Imported Successfully.').'<a href="'.route('vendor-prod-index').'">'.__('View Product Lists.').'</a>'.$log;
    ...
    public function store(Request $request)
    {
        ...
        $customs = [
            'photo.mimes' => __('Photo must be a type of jpeg, jpg, png, svg.'),
            'file.mimes' => __('File must be a type of zip.')
        ];
        ...
        //--- Logic Section
            ...
            if ($file = $request->file('photo'))
            {
                ...
                $file  = Image::make($request->file('photo')->getRealpath());
                $file->orientate();
                $name = time().str_replace(' ', '', $request->file('photo')->getClientOriginalName());
                $file->save('assets/images/products/'.$name);
               $input['photo'] = $name;
           }
    ...
    public function update(Request $request, $id)
    {
        ...
        //--- Redirect Section
        $msg =  $this->languages->{'Product Updated Successfully.View Product Lists.'}.'<a href="'.route('vendor-prod-index').'">'.$this->languages->{'View Product Lists'}.'</a>';
        return response()->json($msg);
        //--- Redirect Section Ends
    }
