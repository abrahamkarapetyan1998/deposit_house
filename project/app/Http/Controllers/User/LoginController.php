...
class LoginController extends Controller
{
    ...
    {
        // $data = Language::findOrFail(Session::get('language'));
      ...
      if (Auth::attempt(['email' => $request->email, 'password' => $request->password])) {
        ...
        if($gs->is_sms_verification == 1){
          if(Auth::guard('web')->user()->sms_verifiy != 1)
          {

            Auth::guard('web')->logout();
            return response()->json(array('errors' => [ 0 => $this->languages->{'Mobile Number is not Verified'} . '<a href="'.route('user-verify-otp',$request->email).'">'.$this->languages->{'Verify Link'}.'</a>' ]));
          }
        }
      ...
      // if unsuccessful, then redirect back to the login with the form data
          return response()->json(array('errors' => [ 0 => __('Credentials Does not Match !') ]));
    }
