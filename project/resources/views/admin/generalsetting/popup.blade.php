                    ...
                    <div class="product-description">
                      <div class="body-area">
                        ...
                        <br>
                          ...
                          @php
                              $popup_data = json_decode($gs->popup_langauge,true);
                          @endphp
                        @foreach (DB::table('languages')->get() as $language)
                        <div class="row justify-content-center">
                          <div class="col-lg-3">
                            <div class="left-area">
                                <h4 class="heading">({{ $language->language }}) {{ __('Popup Title') }} *
                                  </h4>
                            </div>
                          </div>
                          <div class="col-lg-6">
                            <input type="text" class="input-field" placeholder="{{ __('Popup Title') }}" name="language_popup[{{$language->id}}][popup_title]" value="{{ isset($popup_data["$language->id"]["popup_title"]) ? $popup_data["$language->id"]["popup_title"] : '' }}" required="">
                          </div>
                        </div>
                        <div class="row justify-content-center">
                              <div class="col-lg-3">
                                <div class="left-area">
                                  <h4 class="heading">
                                   ( {{ $language->language }}) {{ __('Popup Text') }} *
                                  </h4>
                                </div>
                              </div>
                              <div class="col-lg-6">
                                  <div class="tawk-area">
                                    <textarea class="input-field"  name="language_popup[{{$language->id}}][popup_text]" placeholder="{{ __('Popup Text') }}">{{isset($popup_data["$language->id"]["popup_text"]) ? $popup_data["$language->id"]["popup_text"] : ''}}</textarea>
                                  </div>
                              </div>
                          </div>
                        @endforeach
